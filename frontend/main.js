document.addEventListener("DOMContentLoaded", () => {
  ...
  const PRODUCT_API_BASE_URL = "http://4.237.208.207:8000/";
  const ORDER_API_BASE_URL = "http://4.254.14.118:8001/";
  const CUSTOMER_API_BASE_URL = "http://4.237.247.162:8002/";
    ...
    try {
      ...
      if (customers.length === 0) {
        customerListDiv.innerHTML =
          "<p>No customers available yet. Add some above!</p>";
        return;
      }
      ...
      customers.forEach((customer) => {
        ...
        customerCard.innerHTML = `
                    <h3>${customer.first_name} ${customer.last_name} (ID: ${
          customer.customer_id
                    ...
                    <div class="card-actions">
                        <button class="delete-customer-btn" data-id="${
                          customer.customer_id
                        }">Delete</button>
                    </div>
                `;
      ...
      customerListDiv.innerHTML =
        "<p>Could not load customers. Please check the Customer Service.</p>";
    }
  }
  ...
  customerForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        `Customer "${addedCustomer.email}" added successfully! ID: ${addedCustomer.customer_id}`,
        "success"
      );
  ...
  customerListDiv.addEventListener("click", async (event) => {
    if (event.target.classList.contains("delete-customer-btn")) {
      ...
      if (
        !confirm(`Are you sure you want to delete customer ID: ${customerId}?`)
      ) {
        return;
      }
      try {
        const response = await fetch(
          `${CUSTOMER_API_BASE_URL}/customers/${customerId}`,
          {
            method: "DELETE",
          }
        );
        ...
        if (response.status === 204) {
          showMessage(
            `Customer ID: ${customerId} deleted successfully.`,
            "success"
          );
  ...
  placeOrderForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        "Placing order... (status will update asynchronously)",
        "info"
      );
      ...
      showMessage(
        `Order ${placedOrder.order_id} created with initial status: ${placedOrder.status}. Stock deduction pending.`,
        "success"
      );
                    ...
                    <div class="status-selector">
                        <select id="status-select-${
                          order.order_id
                            ...
                            }>Failed</option> <!-- NEW: Failed Status -->
